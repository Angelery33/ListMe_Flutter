...
class ItemGroupingHelper {
  ...
  static Map<String, List<ItemModel>> groupItems({
    ...
    } else if (list.supportsWishlist) {
      _groupByWishlist(filtered, grouped);
    } else if (list.thematic && list.genreLayoutMode == 1) {
  ...
  static void _groupByStatus(List<ItemModel> items, Map<String, List<ItemModel>> grouped) {
    grouped['Pendientes'] = items.where((i) => i.status == 'PENDING').toList();
    grouped['En Progreso'] = items.where((i) => i.status == 'IN_PROGRESS').toList();
    grouped['Completados'] = items.where((i) => i.status == 'COMPLETED').toList();
    
    // Eliminar grupos vacíos
    grouped.removeWhere((key, value) => value.isEmpty);
  }

  static void _groupByWishlist(List<ItemModel> items, Map<String, List<ItemModel>> grouped) {
  ...
  static void _groupByGenre(List<ItemModel> items, Map<String, List<ItemModel>> grouped) {
